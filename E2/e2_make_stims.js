...
// for (let i = 0; i < 2; i++) {
  ...
  for (const [key, value] of Object.entries(curr_frames)) {
    ...
    // Find the test trial. Same as study trial if it's repeated, otherwise
    // the other alt. 
    let test_alt = curr_alts[j]
    if (!truth_array[i][j]) {
      test_alt = 2 - ((test_alt + 1) % 2)      
    }
    let curr_test_trial = value.find(trial => trial.alt === String(test_alt))
    let test_trial = {}
    ...
    test_trial.repeated = truth_array[i][j]
